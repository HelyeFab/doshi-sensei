...
Our latest image generation model, ``, is a natively multimodal large language model. It can understand text and images and leverage its broad world knowledge to generate images with better instruction following and contextual awareness.
...
3. By providing a file ID (created with the Files API)
...
1. Calculate the number of 32px x 32px patches that are needed to fully cover the image
2. If the number of patches exceeds 1536, we scale down the image so that it can be covered by no more than 1536 patches
3. The token cost is the number of patches, capped at a maximum of 1536 tokens
...
3. Count the number of 512px squares in the image—each square costs a set amount of tokens (see chart below)
...
**A 4096 x 8192 image in "detail": "low" mode costs 85 tokens**
...
For the most precise and up-to-date estimates for image processing, please use our image pricing calculator available [here](https://platform.openai.com/docs/guides/vision#calculating-costs).
