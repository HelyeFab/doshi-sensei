 ...
 * Migrates your IndexedDB data to Firebase
    ...
    try {
      ...
      if (allProgress.length > 0) {
        ...
        allProgress.forEach(progress => {
          const docRef = doc(db, 'users', user.uid, 'textbookVocabularyProgress', progress.id);
          batch.set(docRef, {
            ...progress,
            userId: user.uid,
            lastReviewed: progress.lastReviewed?.toISOString ? progress.lastReviewed.toISOString() : progress.lastReviewed,
            nextReview: progress.nextReview?.toISOString ? progress.nextReview.toISOString() : progress.nextReview,
            createdAt: progress.createdAt?.toISOString ? progress.createdAt.toISOString() : progress.createdAt || new Date().toISOString(),
            updatedAt: progress.updatedAt?.toISOString ? progress.updatedAt.toISOString() : progress.updatedAt || new Date().toISOString()
          });
      ...
      if (allSessions.length > 0) {
        ...
        allSessions.forEach(session => {
          const docRef = doc(db, 'users', user.uid, 'textbookVocabularyStudySessions', session.id);
          sessionBatch.set(docRef, {
            ...session,
            userId: user.uid,
            startTime: session.startTime?.toISOString ? session.startTime.toISOString() : session.startTime,
            endTime: session.endTime?.toISOString ? session.endTime.toISOString() : session.endTime
          });
        });
    ...
    try {
      ...
      if (kanjiProgressData) {
        ...
        if (results.kanjiMastery.found > 0) {
          ...
          kanjiProgress.forEach(progress => {
            const docRef = doc(db, 'users', user.uid, 'kanjiProgress', progress.id || progress.kanji);
            batch.set(docRef, {
              ...progress,
              userId: user.uid,
      ...
      if (kanjiSessionsData) {
        ...
        if (Array.isArray(kanjiSessions) && kanjiSessions.length > 0) {
          ...
          const batch = writeBatch(db);
          kanjiSessions.forEach(session => {
            const sessionId = session.id || `session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            const docRef = doc(db, 'users', user.uid, 'kanjiStudySessions', sessionId);
            batch.set(docRef, {
              ...session,
              userId: user.uid,
      ...
      if (savedItemsData) {
        ...
        if (Array.isArray(savedItems) && savedItems.length > 0) {
          ...
          savedItems.forEach(item => {
            const docRef = doc(db, 'users', user.uid, 'savedStudyItems', item.id);
            batch.set(docRef, {
              ...item,
              userId: user.uid,
      ...
      for (const gameKey of gameKeys) {
        ...
        if (gameData) {
          ...
          try {
            ...
            const docRef = doc(db, 'users', user.uid, 'gameProgress', gameKey);
            await setDoc(docRef, {
              gameId: gameKey,
              data: parsed,
              userId: user.uid,
...
// Auto-run
console.log('Starting migration for user WawMEtfq0dcoVPMr3nuwpFAzr9F2...');
migrateMyData();