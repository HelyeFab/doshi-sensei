...
export async function POST(request: NextRequest) {
  try {
    const { userId, updates, idToken } = await request.json();
    ...
    const result = await serverFirebaseFunctions.fixSubscriptions(
      { userId, updates },
      idToken
    );
    ...
    console.error('Error in admin/fix-subscriptions:', error);
    return NextResponse.json(
