...
export async function POST(request: NextRequest) {
  try {
    const { pricing, idToken } = await request.json();
    ...
    const result = await serverFirebaseFunctions.updatePricingConfig(
      { pricing },
      idToken
    );
    ...
    console.error('Error in admin/pricing-config:', error);
    return NextResponse.json(
