...
export default function MyVideos() {
  ...
  const { checkAndTrack, hasAccess, userType } = useFeature('my_videos', {
    showModal: true,
    showToast: true,
    trackUsage: false // Just checking access, not tracking usage yet
  });
  ...
  const initializeAndLoadVideos = async () => {
    try {
      ...
      await practiceHistoryService.initialize(user?.uid, userType === 'premium');
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-white/80">
                    {userType === 'premium' ? 'Synced across all devices' : 'Saved on this device'}
                  </p>
                </div>
              </div>
