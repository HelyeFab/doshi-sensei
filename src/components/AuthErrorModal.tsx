...
export default function AuthErrorModal({ isOpen, error, type = 'error', onClose }: AuthErrorModalProps) {
  ...
  // For success messages, create a simple format
  const getMessage = () => {
    if (isSuccess) {
      return {
        title: 'Success!',
        message: error, // The 'error' prop contains the success message
        suggestion: 'You will be redirected shortly.'
      };
    }
  ...
  const friendlyMessage = getMessage();
