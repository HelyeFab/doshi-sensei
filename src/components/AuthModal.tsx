...
export default function AuthModal({ isOpen, onClose, defaultMode = 'signin' }: AuthModalProps) {
  ...
  // Close modal when user is authenticated (handles redirect return)
  useEffect(() => {
    if (user && isOpen && !isLoading && !isRedirecting) {
      // User is now authenticated, close the modal
      toast.success('Signed in successfully');
      onClose();
    }
  }, [user, isOpen, isLoading, isRedirecting, onClose]); // Remove toast from deps
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // For redirect flow (mobile or popup blocked), the page will redirect
      // The useEffect above will handle closing the modal when auth completes
      
      // Check if we're using redirect (mobile or popup blocked)
      const isMobile = typeof window !== 'undefined' && window.innerWidth <= 768;
      if (isMobile) {
        setIsRedirecting(true);
        toast.info('Redirecting to Google...');
      }
      // For popup flow on desktop, the auth state will update immediately
      // and the useEffect will handle closing the modal
    } catch (error) {
