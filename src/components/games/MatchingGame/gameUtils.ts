// Game utilities for Matching Game
...
export function createTiles(words: JapaneseWord[], targetPairCount: number): Tile[] {
  const tiles: Tile[] = [];
  // Limit words to what we need (max 3 words per pair for mixed types)
  const maxWordsNeeded = targetPairCount;
  const shuffledWords = [...words].sort(() => Math.random() - 0.5);
  const availableWords = shuffledWords.slice(0, maxWordsNeeded);
  const usedWordIds = new Set<string>(); // Track used words to avoid duplicates
  ...
  // Type 2: Word to reading matching (if kana is available)
