...
// TODO: Replace with new subscription hook when available
...
export default function PokedexContent({ userId, onClose }: { userId?: string; onClose?: () => void }) {
  const { user } = useAuth();
  // TODO: Replace with new subscription hook
  ...
  useEffect(() => {
    const loadCaughtPokemon = async () => {
      try {
        ...
        // TODO: Determine if user is premium based on userType when subscription hook is available
        // For now, assume free users (false)
        const isPremiumUser = false; // userType === 'monthly' || userType === 'yearly';
    ...
    loadCaughtPokemon();
  }, [user]); // TODO: Add userType when available
      ...
      <AnimatePresence>
        {selectedPokemon && (
          <motion.div
            ...
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
            className="absolute bottom-0 left-0 right-0 bg-card rounded-t-3xl shadow-2xl p-6 z-50"
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground">#{selectedPokemon.toString().padStart(3, '0')}</p>
              </div>
            </div>
          </motion.div>
        )}
