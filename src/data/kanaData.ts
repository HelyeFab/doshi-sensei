...
export const kanaData: KanaCharacter[] = [
  ...
  // Y-consonant combinations (digraphs)
...
// Helper to get kana by row
export const getKanaByRow = (row: string): KanaCharacter[] => {
  return kanaData.filter(kana => kana.row === row);
};

// Helper to get basic kana (exclude digraphs)
export const getBasicKana = (): KanaCharacter[] => {
  return kanaData.filter(kana => kana.type !== 'digraph');
};

// Helper to get digraphs only
export const getDigraphs = (): KanaCharacter[] => {
  return kanaData.filter(kana => kana.type === 'digraph');
};

// Row order for display
export const kanaRowOrder = [
  'vowel', // a, i, u, e, o
  'k', 'g', // ka row & ga row
  's', 'z', // sa row & za row
  't', 'd', // ta row & da row
  'n', // na row
  'h', 'b', 'p', // ha row, ba row, pa row
  'm', // ma row
  'y', // ya row
  'r', // ra row
  'w', // wa row
...
// Digraph row order
export const digraphRowOrder = [
  'ky', 'gy', // kya row & gya row
  'sh', 'j', // sha row & ja row
  'ch', // cha row
  'ny', // nya row
  'hy', 'by', 'py', // hya row, bya row, pya row
  'my', // mya row
  'ry' // rya row
];