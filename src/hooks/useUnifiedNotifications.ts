...
export function useUnifiedNotifications() {
  ...
  useEffect(() => {
    ...
    const handleServiceWorkerUpdate = () => {
      ...
      toast.info(
        'Update Available',
        'New version ready with latest features. Refresh to update.'
      );
    };
    ...
    // Initial setup - delay to avoid setting state during render
    wasOffline.current = !navigator.onLine;
    
    // Use setTimeout to defer the initial network check
  ...
  }, [toast]); // Removed networkInfo.quality to prevent re-renders
