...
async function initializeAdmin(): Promise<void> {
  ...
  initializationPromise = (async () => {
    try {
      ...
      // Method 1: Use generated credentials file (for Netlify production)
      // This file is created at build time by inject-firebase-creds.js
      if (!initialized) {
        try {
          const { FIREBASE_CREDENTIALS } = await import('./firebase-credentials-generated');
          
          if (FIREBASE_CREDENTIALS && isValidServiceAccount(FIREBASE_CREDENTIALS)) {
            adminApp = admin.initializeApp({
              credential: admin.credential.cert(FIREBASE_CREDENTIALS as admin.ServiceAccount),
              projectId: FIREBASE_CREDENTIALS.project_id || projectId,
              storageBucket: 'doshi-sensei',
            });
            initialized = true;
            console.log('[Firebase Admin] Initialized from generated credentials');
          }
        } catch (importError) {
          // File doesn't exist or couldn't be imported (normal in development)
          console.log('[Firebase Admin] Generated credentials not found, trying other methods');
        }
      }
      ...
      // Method 3: Full service account JSON in environment variable
      ...
      // Method 3: Individual environment variables (server-side only)
      if (!initialized) {
