...
// TODO: Add stats tracking when stats system is implemented
...
class PokemonManager {
  ...
  async saveCaughtPokemon(
    ...
    // Always save to localStorage for offline access with user identification
    await pokemonStorage.savePokemonLocally(pokemonId, jlptLevel, kanjiIds, userId, userEmail);

    // Track Pokemon catch in stats system (when implemented)
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        try {
          ...
          // Refresh Pokemon count in stats tracker (when implemented)
